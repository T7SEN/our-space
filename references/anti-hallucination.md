...
- **`/api/cron/ritual-windows`** is the only cron endpoint and the only place writing `ritual:fcm:sent:*`. Triggered from TWO sources in parallel: cron-job.org (minute cadence, primary, configured out-of-band in the operator's account) AND `vercel.json` Vercel Cron (daily on Hobby, fallback). Dedup makes parallel fires safe. Don't propose calling it from feature code, removing either trigger, or removing the `CRON_SECRET` bearer check.
- **Vercel tier is Hobby; minute-cadence cron lives at cron-job.org.** Don't write code that assumes Vercel Cron alone can deliver minute cadence — it can't on Hobby. Don't propose deleting `vercel.json` either; the Hobby daily tick is a free fallback for cron-job.org outages.
